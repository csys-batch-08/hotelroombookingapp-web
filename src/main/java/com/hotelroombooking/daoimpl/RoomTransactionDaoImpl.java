...
public class RoomTransactionDaoImpl implements RoomTransactionDao {
	...
	public boolean bookRoom(HttpSession session) {
		...
		try {
			...
			if (vacantRoomNumber != 0) {
				try {
					String bookRoomQuery = "insert into room_transaction(room_number,check_in,check_out,category,location,guest_id)"
							+ " values(?,?,?,?,?,?)";
		...
		try {
			...
			if (vacantRoomNumber != 0) {
				try {
					...
					String updateRoomQuery = "update room_transaction set check_in=?,check_out=?,category=?,location=? where "
							+ "room_number=?";
				...
				try {
					conn = ConnectionUtil.getDbConnection();
					String updateRoomQuery2 = "update room_transaction set room_number=? where check_in=? and check_out=? "
							+ "and category=? and location=? and guest_id=?";
	...
	/**
	 * method to list the room bookings
	 */
	@Override
	...
	/**
	 * method to find the booking room price
	 */
	@Override
	...
	/**
	 * method to find the updated room price
	 */
	@Override
